...
for num in "$@"; do
	...
	if [[ $num == *"_1.fastq" ]]; then
		...
		if [ "$preload" = true ]; then
			CMD="kraken2 --use-names --db ${KRAKEN2_DB} --paired --threads $threads --output ${out}/${sample}.krak2 --report ${out}/${sample}.kreport $reads_1 $reads_2"
		else
			CMD="kraken2 --use-names --db ${KRAKEN2_DB} --paired --threads $threads --output ${out}/${sample}.krak2 --report ${out}/${sample}.kreport --memory-mapping $reads_1 $reads_2"
		fi
	...
	if [[ $num == *"fa" ]] || [[ $num == *"fasta" ]]; then
		...
		if [ "$preload" = true ]; then
			CMD="kraken2 --use-names --db ${KRAKEN2_DB} --threads $threads --output ${out}/${sample}.krak2 --report ${out}/${sample}.kreport $num"
		else
			CMD="kraken2 --use-names --db ${KRAKEN2_DB} --threads $threads --output ${out}/${sample}.krak2 --report ${out}/${sample}.kreport --memory-mapping $num"
		fi
