...
# Description --------------------------------------------------------------------------------------------------------------
    ...
    # 1. Performs AMR gene annotation of clean ONT reads using RGI-BWT (single-end mode)
    ...
    # ONT-specific notes:
    #   - Input is a single uncompressed FASTQ (not paired-end)
    #   - Uses --read_one only (no --read_two); KMA aligner handles single-end reads
    #   - Otherwise matches the short-read 5.4_rgi_bwt.sh behavior
    #   - Output: ${rgi_bwt_dir}/kma_output/${ID}.rgi_kma.txt
...
# Print script information to log ------------------------------------------------------------------------------------------
    H1 "Description: 5.4_rgi_bwt.sh (ONT single-end)"
        echo -e "This script does the following:"
        echo -e "1. Performs AMR gene annotation of clean ONT reads using RGI-BWT (single-end, KMA aligner)"
    ...
    H1 "Variables"
        ...
        H2 "Input"
            datasetROOT=$(pwd)
            ONT=${clean_readDir}/${ID}_ont.fastq; echo $ONT
            aligner=kma

        H2 "Output"
            if [[ ! -d ${moduleDir}/${aligner}_output ]]; then mkdir -p ${moduleDir}/${aligner}_output; fi
...
# Run RGI-BWT (single-end ONT) -----------------------------------------------------------------------------------------
    H1 "Running RGI-BWT (ONT single-end)"
    if [[ ! -f "${outputFile}" ]]; then
        ...
        # Set up temp directory in scratch
            ...
            cd $CARD_DB
            mkdir -p ${ID}_${aligner}_ont
            cd ${ID}_${aligner}_ont
        ...
        # Run rgi bwt (single-end: --read_one only, no --read_two)
            if [[ ! -f "${ID}.gene_mapping_data.txt" ]]; then
                comment "Run RGI-BWT for the first time (single-end ONT)."
                rgi bwt \
                --read_one ${ONT} \
                -a $aligner \
                --output_file ${ID} \
            ...
            elif [[ $(cat ${ID}.gene_mapping_data.txt | wc -l) -eq 1 ]]; then
...
# Clean up temp output from scratch directory
    if [[ ! -f "$outputFile" ]] || [[ $(cat $outputFile | wc -l) -le 1 ]]; then
        ...
        if [[ -d ${CARD_DB}/${ID}_${aligner}_ont ]]; then
            comment "Removing tmp scratch directory"
            rm -r ${CARD_DB}/${ID}_${aligner}_ont
        fi
    fi
