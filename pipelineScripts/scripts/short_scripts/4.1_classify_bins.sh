...
# Description --------------------------------------------------------------------------------------------------------------
    # This script is assigns taxonomy to consolidated, reassembled bins for each sample using GTDB-tk.
...
# Print script information to log ------------------------------------------------------------------------------------------
    H1 "Usage"
    comment "This script is assigns taxonomy to consolidated, reassembled bins for each sample using GTDB-tk."
    ...
    H1 "Variables"
        ...
        H2 "Output"
            ...
            echo -e "${out}/classify/gtdbtk.bac120.summary.tsv"
...
H1 "Classify bins"
    ...
    # metawrap classify_bins -b $BINS -o $OUT -t $SLURM_CPUS_PER_TASK
    
    conda deactivate
    module unload anaconda3/2023.09

# Completion status
    if [[ -s ${moduleDir}/${ID}/classify/gtdbtk.bac120.summary.tsv ]]; then
